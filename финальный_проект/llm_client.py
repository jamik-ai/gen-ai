        ...
        **kw: Any,
        ...
        def _call(thinking: dict):
            ...
            try:
                ...
                safe = {k: v for k, v in merged.items() if k != "reasoning_effort"}
                return self._c.chat.completions.create(
                    model=model,
                    messages=msgs,
                    response_format={"type": "json_object"},
...
class _RawCompletions:
    ...
    Нужен для tool-calling агента (Responder), где response_model-обёртка
    из _Completions не подходит — там используется `tools=`/`tool_choice`,
    а не `response_format={"type": "json_object"}`.
    """
