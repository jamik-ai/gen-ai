...
ДЗ5: добавлен compare_periods, JSONL-лог шагов (trace.jsonl).

Запуск:
    python agent.py "Во сколько раз вырос курс USD с января 2022 по апрель 2026?"
    python agent.py --parallel "Сравни курс USD сегодня и 2 января 2022"
...
# ДЗ5 задание 2: запись шагов в JSONL
def _write_trace(run_id: str, record: dict) -> None:
    entry = {"run_id": run_id, "ts": datetime.datetime.now().isoformat(), **record}
...
- compare_periods: сравнить метрику в двух периодах (возвращает delta и ratio)

Алгоритм:
1. Если вопрос «вырос/упал/во сколько раз» — используй compare_periods напрямую.
2. Если нужны отдельные числа — запрашивай get_* инструментами.
3. Арифметику считай ТОЛЬКО через calculate, даже если кажется тривиальной.
...
def _exec_one(tc, cache: Optional[dict] = None) -> tuple[Any, dict, dict]:
    ...
    try:
        ...
        return tc, args, {"error": f"плохие аргументы для {name}: {e}"}
...
def run_agent(
    ...
    *,
    ...
    """ReAct-цикл с JSONL-логированием шагов (ДЗ5 задание 2)."""
    ...
    for step in range(1, max_iter + 1):
        ...
        if not msg.tool_calls:
            record = {"step": step, "final": msg.content}
            trace.append(record)
            _write_trace(run_id, record)
            return _finish(
        ...
        if others:
            ...
            for tc, args, obs in results:
                ...
                record = {"step": step, "call": tc.function.name, "args": args, "obs": obs}
                trace.append(record)
                _write_trace(run_id, record)
                if verbose:
                    print(
